...
function nospace( board ){  /*存在0，有空间*/
...
function canMoveLeft(board){
	for(var i=0; i<4;i++)
		for(var j=0; j<4;j++)
			if(board[i][j]!=0)
				if(board[i][j-1]==0 || board[i][j-1]==board[i][j])   /*左侧为0或者左侧相等*/
					return true;
...
function noBlockHorizontal(row,coll,col2,board){   /*第row行，从col1到col2，有没有障碍物*/
...
function nomove(board){  /*不能再向任何方向移动了*/
